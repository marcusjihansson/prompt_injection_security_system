...
class SecurityValidator:
    ...
    def load_test_dataset(self):
        ...
        dataset_path = (
            Path(__file__).parent.parent / "tests" / "prompt_attacks" / "prompt_injections.json"
        )
    ...
    def run_validation_suite(self, max_samples_per_category: int = 5):
        ...
        print("=" * 60)
        print(f"Testing {max_samples_per_category} samples per category for quick validation")
        ...
        for category in categories:
            ...
            samples = dataset[category][:max_samples_per_category]  # Limit samples for testing
            ...
            for i, sample in enumerate(samples):
                ...
                if result["blocked"]:
                    blocked_count += 1
                    print(f"    🚫 BLOCKED: {result.get('detection_method', 'unknown')}")
                else:
                    print(f"    ✅ ALLOWED: {result.get('detection_method', 'unknown')}")
    ...
    def save_results(self, output_path=None):
        ...
        if output_path is None:
            ...
            output_path = f"results/security_validation_{timestamp}.json"
...
def main():
    ...
    try:
        ...
        try:
            summary = report["summary"] if isinstance(report, dict) and "summary" in report else {}
            ...
            print(f"Blocked: {summary.get('blocked_tests', 0) if isinstance(summary, dict) else 0}")
            print(
