...
class RegexBaseline:
    ...
    def __init__(
        self,
        patterns_by_type: Optional[Dict[ThreatType, List[str]]] = None,
        ...
        self.compiled = {
            t: [re.compile(p, re.IGNORECASE) for p in ps] for t, ps in patterns_by_type.items()
        }
    ...
    def check(self, text: str) -> RegexResult:
        """Check text for threats using regex patterns with early exit optimization."""
        threats = set()
        matches = {}
        ...
        for ttype in self.high_severity:
            ...
            for pat in patterns:
                ...
                if m:
                    ...
                    return RegexResult(threats={ttype}, severity=3, matches={ttype: [m.group(0)]})

        # Continue with remaining patterns if no high-severity threat found
        for ttype, patterns in self.compiled.items():
            if ttype in self.high_severity:
                continue  # Already checked

            # Optional prefilter (soft) - future optimization spot
            _ = self.substrings.get(ttype, None)
